...
//API key
const apiKey = "6b5a4e9eaa01742faf57f91ca59c8188";
...
form.addEventListener("submit", (e) => {
  ...
  if (listItemsArray.length > 0) {
    const filteredArray = listItemsArray.filter((el) => {
      let content = "";
      let cityName = el.querySelector(".city").textContent.toLowerCase();
      console.log("hello");
    ...
    .then((data) => {
      ...
      const markup = `
				...
				<div>
					...
					<h3><span class="city">${name}</span><span class="country">${
        sys.country
      }</span></h3>
				</div>
			`;
